...
- **结果落盘**：原始样本保存到 `results/{timestamp}_{mode}_{stream|nonstream}.json`
...
# 非交互固定并发：并发 8，总请求 50
./llm_pressure -t 8 -n 50

# 指定 provider / model 后直接运行
./llm_pressure -p example-provider -m glm-5.2 -t 8 -n 50
```
...
两种模式都会实时显示进度（已完成 / 成功 / 失败 / 累计 token），结束后终端打印统计表，并保存 JSON 到 `results/`。
...
- `last_model` 会在每次运行后自动更新，用于 `./llm_pressure -t 8 -n 50` 这类非交互运行
