...
# 一次测试多个模型
./llm_pressure -p example-provider -m glm-5.2,kimi-2.6 -t 8 -n 50
./llm_pressure -p example-provider -m glm-5.2 -m kimi-2.6 -t 8 -n 50
```
...
- `last_model` 会在每次运行后自动更新；多模型会保存为逗号分隔列表，用于 `./llm_pressure -t 8 -n 50` 这类非交互运行
