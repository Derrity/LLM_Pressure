...
- **持续高消耗模式**：`-burn` 使用长输出 prompt 和更大的 `max_tokens`，默认一直跑到 Ctrl+C，也可用 `-d` 或 `-n` 限制
...
# 持续高 token 消耗：并发 8，一直跑到 Ctrl+C
./llm_pressure -p example-provider -m glm-5.2 -t 8 -burn

# 高消耗跑 30 分钟后自动停止
./llm_pressure -p example-provider -m glm-5.2 -t 8 -burn -d 30m
...
# 只列出可用 model ID，不执行压测
./llm_pressure -p example-provider -m
./llm_pressure -u https://api.example.com/v1 -m
...
- `-t`：固定并发线程数
- `-n`：总请求数，默认 `32`；固定并发下可设为 `0` 表示一直运行直到 Ctrl+C
- `-d`：固定并发运行时长，例如 `30s`、`10m`、`1h`；不能和 `-n` 同时使用
- `-max-tokens`：每次请求的 `max_tokens`，默认 `512`
- `-burn`：持续高 token 消耗模式，使用长输出 prompt，默认 `max_tokens=4096`，且未传 `-n/-d` 时一直运行直到 Ctrl+C
...
- 列模型：`-p example-provider -m` 或 `-u https://api.example.com/v1 -m` 只打印可用 model ID，不执行压测
...
这会以 8 并发持续请求，每次请求尽量生成长文本。按 Ctrl+C 后程序会停止发新请求，并输出已经采集到的吞吐、延迟、token 总量和错误统计。
